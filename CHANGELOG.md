...
- **Optional HTML report.** `filecap report --html` writes a self-contained `files.html` alongside the CSV. Same 32 columns as the CSV; rendered as a sortable (click-header), filterable (search input), and print-friendly table with no external dependencies. Image-only PDFs are highlighted with a yellow row background; flagged filenames get a left accent border. `writeHtml` is exported from the package main. The `filecap_report` MCP tool gains an optional `html: boolean` parameter. Both audit scripts (`audit-remote.sh`, `audit-fleet.sh`) prompt the auditor to opt in and propagate the flag through.
