...
function buildPageUrl(base, plural, limit, start) {
  return (
    `${base}/api/${plural}` +
...
export async function fetchAllEntries(restApiBase, plural, fetcher, options = {}) {
  ...
  let response;
  try {
    response = await fetcher(buildPageUrl(base, pluralToUse, limit, 0));
  } catch (err) {
    const kebab = kebabize(plural);
    if (/HTTP 404/.test(err.message) && kebab !== plural) {
      response = await fetcher(buildPageUrl(base, kebab, limit, 0));
      pluralToUse = kebab;
    } else {
      throw err;
    }
