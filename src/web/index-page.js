...
export function renderStatusDot(status, key) {
  if (status !== "live" && status !== "down") return "";
  const label = status === "live" ? "Site live" : "Site unreachable";
  const title = status === "live"
    ? "This site responded at the last check"
    : "This site was unreachable at the last check";
  // v1.22.1: a compact pill pinned to the card's upper-right corner — dot +
  // label, plus a "checked <Chicago time>" line the on-demand client fills
  // (data-uptime-checked). data-uptime-key lets the client find + update it.
  const keyAttr = key ? ` data-uptime-key="${he(String(key))}"` : "";
  return `<div class="status-dot status-${status}"${keyAttr} title="${he(title)}"><span class="status-line"><span class="status-glyph" aria-hidden="true"></span><span class="status-label">${label}</span></span><span class="status-checked" data-uptime-checked></span></div>`;
}
...
export function renderToolCard(tool, { showStatus = false } = {}) {
  ...
  ${showStatus ? renderStatusDot(tool.status, tool.name) : ""}
