...
export function renderStatusDot(status, key) {
  ...
  // v1.22.0: data-uptime-key lets the on-demand client (uptime-client.js) find
  // and live-update this line from the uptime function. Omitted when no key.
  const keyAttr = key ? ` data-uptime-key="${he(String(key))}"` : "";
  return `<p class="status-dot status-${status}"${keyAttr} title="${he(title)}"><span class="status-glyph" aria-hidden="true"></span><span class="status-label">${label}</span></p>`;
}
...
<footer class="site-footer">
  ...
  <span class="uptime-stamp" data-uptime-stamp></span>
