...
```bash
cd /Volumes/satechi/webdev/icjia-fleet-audit
mkdir -p /tmp/icjia-fleet-audit-pdf
node -e "
import('pdf-lib').then(async ({PDFDocument, StandardFonts}) => {
  ...
  await fs.writeFile('/tmp/icjia-fleet-audit-pdf/sample.pdf', await doc.save());
});
"
./bin/filecap.js scan /tmp/icjia-fleet-audit-pdf -o /tmp/icjia-fleet-audit-pdf-out.ndjson --no-hash
cat /tmp/icjia-fleet-audit-pdf-out.ndjson | head -2 | tail -1 | node -e "
...
"
rm -rf /tmp/icjia-fleet-audit-pdf /tmp/icjia-fleet-audit-pdf-out.ndjson
```
...
[0.2.0]: https://github.com/ICJIA/icjia-fleet-audit/releases/tag/v0.2.0
```
