...
**1. Navbar.** ICJIA wordmark on the left; action buttons on the right — an amber `Start here` ([/help](https://fleet.icjia.app/help)) first, then `Accessibility FAQs` ([accessibility.icjia.app](https://accessibility.icjia.app)) and `File Audit Tool` ([audit.icjia.app](https://audit.icjia.app)), plus internal `What's New` and `Search` links. The wordmark is hidden between 601px and 1180px, where six buttons no longer fit beside it.
