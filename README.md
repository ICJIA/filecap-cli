...
**Language and staff workflow.** Manager-facing strings use *"may need"* rather than prescriptive *"needs"* — the Fleet Audit describes what the data suggests, the audit team decides what to do. Every workbook and CSV the pipeline emits carries two staff-fill columns at the far right: `Delete?` (column `S`) and free-text `Notes` (column `T`). They are absent from the web views — the downloaded file is the artefact staff edit and hand back, and it is the record the audit team keeps. As of v1.61.0 `/help` teaches **`Notes` only** — one of three outcomes per file (`archive`, `remediate`, `as-is`), reached by opening the file and the page that carries it — with `Delete?` left blank.
