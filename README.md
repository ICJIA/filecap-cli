...
**1. Navbar.** ICJIA wordmark on the left; navigation on the right. As of v1.61.2 only `Help` ([/help](https://fleet.icjia.app/help)) is a button — amber, first, and the one link that has to be noticed. Everything beside it is a plain text link at 0.78rem: `Sites`, `Accessibility FAQs` ([accessibility.icjia.app](https://accessibility.icjia.app)), `File Audit Tool` ([audit.icjia.app](https://audit.icjia.app)), `Search`, and `What's New`. Six filled pills read as six competing calls to action and crowded the wordmark off the line; the row now wraps instead of collapsing to unlabelled icons. **The footer carries every header destination** plus the in-bundle `Accessibility log`, so a reader who scrolled to the bottom looking for something always finds it.

**2. Fleet hero.** Leads with the **audit count** in big amber type (at the current snapshot: "4,633 of 8,768 files may need accessibility audit") with a donut chart showing the audit-share percentage and an estimate of the document-page workload. As of v1.62.0 the headline block defines **remediation in one plain sentence** (fixing a document so a screen-reader user can read it) and carries a **"Find your website ↓"** jump button — the site cards sit directly below, with the methodology sections (why-not-all-files, by-file-type, PII) moved *after* the cards.

**3. What's New banner.** A dismissible announcement of the newest change, capped at a **two-sentence summary** with a "Read the full update" link (v1.62.0) and backed by the full newest-first archive at [/whats-new](https://fleet.icjia.app/whats-new). Dismissal is permanent per entry (`localStorage`).
...
**9. Help — [/help](https://fleet.icjia.app/help).** The optional guide for staff who have been handed a website and told to review its files. Five numbered steps as a stepper, nothing collapsible: find your website, download its spreadsheet, see what is in it, open the two link columns to decide what happens to each file (`archive`, `remediate`, or `as-is`, recorded in `Notes`), and email the workbook to `christopher.schweda@illinois.gov`. The deciding rests on `Public URL` (column `D`, opens the file) and `Page References` (column `E`, opens the page carrying it) — a file cannot be judged from its name. Its centrepiece is a **column map** — the workbook's twenty columns drawn to scale, fifteen dimmed and five lit (`B` File name, `D` Public URL, `E` Page References, `I` Score, `T` Notes) — which answers "what is all this?" and "why can't I find the Notes column?" in one picture. It also carries a **How this differs from SiteImprove** section — the two are framed as complementary and the page says outright that nobody has to choose (a page scanner scores pages; this audits the documents those pages publish), with the conformance target stated plainly as WCAG 2.1 Level AA per ADA Title II and IITAA 2.1, and the in-house/statewide difference framed as a trade: a vendor-maintained statewide platform serves every Illinois agency, so an ICJIA-specific request lands faster here. Linked first in the header nav on every page, from the shared footer, and from a callout at the top of the fleet index.

**10. By-file-type drill-down.** Click "PDFs" → a detail page listing every PDF across the fleet. Same for Word, Excel, PowerPoint, legacy Office, images, text, archives, web files, and other — plus **File errors** (documents the scorer couldn't process, with reasons) and **Orphaned files** (no known referring page) reports, each with a matching workbook.

**Language and staff workflow.** Manager-facing strings use *"may need"* rather than prescriptive *"needs"* — the Fleet Audit describes what the data suggests, the audit team decides what to do. Every workbook and CSV the pipeline emits carries one staff-fill column at the far right: free-text `Notes` (column `S`). It is absent from the web views — the downloaded file is the artefact staff edit and hand back, and it is the record the audit team keeps. `/help` teaches one of three outcomes per file (`archive`, `remediate`, `as-is`), reached by opening the file and the page that carries it. **v1.66.0 removed the former `Delete?` column**: nothing published on a State website can be destroyed under records-retention policy, so a column inviting "flag for removal" named an outcome that never existed, and reviewers read it as a fourth choice competing with the three real ones. `archive` — off the website, still retained — is as far as a file goes.
...
A fully static web app (pure HTML + CSS + vanilla JS, no framework) generated by a Node.js ESM pipeline, run from the GitHub repo (npm package deprecated as of v1.13.0). The pipeline walks a directory tree (concurrent-bounded), produces line-delimited JSON (NDJSON): a header line, one entry per file, a footer line. Each entry includes filesystem metadata + SHA-256 hash + format-specific introspection (pdfjs-dist for PDFs, jszip + fast-xml-parser for DOCX, exceljs for XLSX) and, after the `audits` pass, an `entry.audit` accessibility grade/score from audit.icjia.app (PDFs since 1.9.0; modern Office since 1.54.0). 21-column CSV writer (`CSV_COLUMNS` is the single source of truth: 20 value columns — `Public URL`, `Page References` page-links (1.8.0), `Audit Report` link + `Remediation Score` grade/score (1.9.0/1.34.0), sortable numeric `Score (0-100)` + `Grade` (1.43.0), `Page Count` (1.20.0), etc. — plus the csvOnly `Notes` staff-fill column from v1.7.16) feeding the CSV, self-contained dark-mode HTML report pages (sortable/filterable client-side JS), and XLSX workbooks. Cross-server rollup with content-duplicate detection via SHA-256.

Also includes an MCP server (the `mcp` subcommand) exposing five tools — `filecap_scan`, `filecap_rollup`, `filecap_report`, `filecap_query_inventory`, `filecap_web_rollup` — **now deprecated**: the current product is the fleet audit + web rollup, so the `mcp` subcommand is no longer part of the normal workflow (kept for compatibility; filesystem access is default-deny to `~/filecap-audits` since v1.63.0).
...
ESM-only. Node 20+ required. 108 test files; 1,724 tests via vitest. Source under `src/`; entrypoint `bin/filecap.js`. License: MIT.
...
**CSV / HTML / XLSX column layout.** `CSV_COLUMNS` (`src/report/csv.js`) is the single source of truth for all three writers. Entries flagged `csvOnly: true` (the `notes` column) are filtered out of the HTML and XLSX views, so the CSV ships all 21 columns while the HTML renders a manager-trimmed projection of the non-`csvOnly` columns and the XLSX reprojects them into a manager-friendly order (`XLSX_COLUMN_ORDER`). The `Audit Report` (report link) and `Remediation Score` (`grade/score`, e.g. `B/88`) columns come from `entry.audit`.
...
- **Staff-fill column (CSV-only, since v1.7.16)**: `Notes` (free-text) — one of `archive` / `remediate` / `as-is` plus the reviewer's reasoning. It doesn't appear in the HTML view; the web is informational, the workbook is the actionable artefact. The `Delete?` sibling was removed in v1.66.0 (records retention makes deletion impossible; `archive` is the furthest outcome).
...
- **SSRF guard on scraped-URL fetches (v1.63.0).** The build-time fetches that reach URLs derived from scraped content — recursive sub-sitemap `<loc>` values and `og:image`/page URLs — go through `src/util/safe-fetch.js`, which refuses loopback / link-local / private / cloud-metadata hosts and non-http(s) schemes and fetches with `redirect: "manual"` so a redirect can't be chased into an internal address (FC-2026-038).
- **No credential across a redirect (v1.63.2).** The Strapi auth fetcher, the `bearerLogin` refresh POST, and both audit-API scorers use `redirect: "manual"`, so a bearer token or the login password can never be forwarded to another origin on a 3xx (FC-2026-040, FC-2026-041).
- **`gitRepo` validation (v1.63.2).** A `type:"git"` site's `gitRepo` must be a canonical `https://github.com/<owner>/<repo>` URL before it reaches `git clone`, closing git's `ext::` transport and leading-`-` option-injection vectors (FC-2026-042).
- **Local-only error logs.** The per-run error log (`errors-<runId>.ndjson`/`.csv`, v1.63.0) is written under `~/filecap-audits/_runs/` and is **never** part of the deployed bundle, so verbose error context can't reintroduce the origin-infrastructure exposure removed by FC-2026-033. Secrets are redacted from every record.
- **MCP scan path restriction (default-deny since v1.63.0).** The `mcp` subcommand is **deprecated** (the current product is the fleet audit + web rollup); when run, filesystem access is confined to the default audits base (`~/filecap-audits`) unless `FILECAP_MCP_ALLOWED_PATHS` (colon-separated absolute paths) is set to widen it, and a stderr notice at startup names the active restriction (FC-2026-039).
...
### 2026-08-24 — pipeline security review (v1.63.0–v1.63.2)

A red/blue review of the live product — the pipeline (scan → references/sitemap → og-meta → audits → introspect) and the deployed web-rollup bundle. **One real bug (Medium) and one posture fix closed in v1.63.0; four low residuals all closed in v1.63.2. 0 exploitable XSS, 0 credential leaks.** Full detail in [`docs/security/audit-2026-08-24.md`](docs/security/audit-2026-08-24.md). Also confirmed clean: no XXE (fast-xml-parser resolves no external entities), no zip-slip (in-memory only), pdfjs runs with `isEvalSupported:false` and no scripting, and every `href` passes a `safeUrl()` scheme gate.

| # | Severity | Finding | Status |
|---|---|---|---|
| FC-2026-038 | Medium | **Server-side SSRF via scraped URLs** — recursive sub-sitemap `<loc>` values and `og:image`/page URLs were fetched with no host guard; a malicious fleet site could point them at `169.254.169.254`, `127.0.0.1`, or a LAN address | **Fixed in v1.63.0** — new `src/util/safe-fetch.js` blocks loopback/link-local/private/metadata hosts + non-http(s) and fetches `redirect:"manual"` (no redirect chasing); both fetch sites route through it |
| FC-2026-039 | Low (posture) | **MCP server exposed the whole filesystem by default** when `FILECAP_MCP_ALLOWED_PATHS` was unset | **Fixed in v1.63.0** — default-deny to the audits base (`~/filecap-audits`); the MCP subcommand is deprecated/unused regardless |
| FC-2026-040 | Low | Credentialed fetchers relied implicitly on undici stripping `Authorization` across a cross-origin redirect | **Fixed in v1.63.2** — the Strapi auth fetcher + both audit scorers POST with `redirect:"manual"` |
| FC-2026-041 | Low | The `bearerLogin` refresh POST could carry the cleartext password across a 307/308 redirect (undici strips headers, not the body) | **Fixed in v1.63.2** — the `/auth/local` POST uses `redirect:"manual"` |
| FC-2026-042 | Low | `gitRepo` reached `git clone` with no scheme validation (`ext::sh -c …` / leading-`-` option injection) | **Fixed in v1.63.2** — restricted to `https://github.com/<owner>/<repo>` via `isValidGitRepoUrl` before git runs |
| FC-2026-043 | Low | The website score was interpolated into HTML without a local escape | **Fixed in v1.63.2** — site + per-page scores escaped, so a malformed sidecar renders as inert text |

A netlify-cli **v26** compatibility fix also shipped in v1.63.1: `netlify deploy` now runs a build unless `--no-build` is passed, so the deploy path (`runNetlifyDeploy`) passes it for the pre-built static bundle.
...
| FC-2026-035 | Note | Per-file `absolutePath` still carries the Forge path for strapi files in `audit.xlsx` + `audit-fleet.ndjson` (git's `absolutePath` is the functional GitHub URL, so it can't be blanket-dropped) | **Fixed in v1.64.1** — `safeAbsolutePath` keeps only an `http(s)` value (a git site's GitHub URL) and blanks the Strapi/Forge filesystem path, applied in `buildRow` (every workbook) and `stripPiiFromEntry` (the consolidated NDJSON). Verified on rebuild: **8,743 → 0** `/home/forge` in `audit-fleet.ndjson`, **0** across all `.xlsx`, git GitHub URLs kept |
...
**v1.69.0 shipped (2026-08-28).** The Fleet Audit is a **web app at [fleet.icjia.app](https://fleet.icjia.app)**, republished from every audit run. At the current snapshot: **6,716 files** inventoried across the 11 audited content sites, **3,217** of them (48%) on the remediation list, an estimated ≈41,354 document pages of workload. (The Document Archive — archive.icjia.cloud, 2,421 files — is temporarily out of the audit's scope while its contents are reorganized: `excluded: true` in sites.json, v1.69.0; deleting the flag brings it back.) Every PDF and modern Office document carries a per-file `grade/score` (e.g. `B/88`) with a linked audit report, and every site carries an independent axe-based **Website accessibility** score alongside its **File accessibility** score. The app includes fleet-wide [/search](https://fleet.icjia.app/search) with custom Excel reports, the [/sites](https://fleet.icjia.app/sites) directory with live uptime, a What's New announcement system, and per-site XLSX workbook downloads. See the [CHANGELOG](CHANGELOG.md) for the version-by-version breakdown.
...
| vNext | deferred | Headless rendering for SPA sites where Strapi fallback isn't sufficient (the once-planned `process-deletions` subcommand is dropped — records retention means files are never deleted) |
...
| File | Purpose |
|---|---|
| `audit-file-list.csv` | One row per file, 21 columns (20 value columns — including `Page References`, `Audit Report`, `Remediation Score`, sortable numeric `Score (0-100)` + `Grade`, and `Page Count` — plus the `Notes` staff-fill column). Human-readable column headers. Filterable in Excel, Smartsheet, etc. |
...
**CSV column order** (21 columns; `Page References` added at position 5 in v1.8.0, `Audit Report` at 6 in v1.9.0, `Page Count` restored in v1.20.0, `Remediation Score` added in v1.34.0, numeric `Score (0-100)` + `Grade` in v1.43.0, `Delete?` removed in v1.66.0; the `Notes` staff-fill column stays last):

`Server, Website, Server IP, Public URL, Page References, Audit Report, Date published, Source folder on server, File location (relative to source folder), Full file path on server, File name, Page Count, File extension, File type, Size (bytes), Content hash (SHA-256), Duplicate of, Remediation Score, Score (0-100), Grade, Notes`
...
**The last column is CSV-only and meant for staff input.** `Notes` is empty by default; the reviewer writes one of three outcomes — `archive`, `remediate`, `as-is` — followed by their reasoning (*archive — superseded by the 2026 edition*). When the edited workbook comes back, the audit lead reads the notes, acts on them, and re-runs the audit. It doesn't appear in the HTML table view — the web view is informational, the workbook is the actionable artefact.

There is deliberately **no delete option**. Nothing published on a State website can be destroyed under records-retention policy, so `archive` — take it off the website, retain the file — is the furthest a decision goes. v1.66.0 removed the former `Delete?` column for exactly this reason: it advertised an outcome that was never available and competed with the three that are.
...
- **`audit-file-list.csv`** — The main deliverable. One row per file, 21 columns covering server, website, file location, full path, public URL, page references, filename, page count, type, size, content hash, duplicate-of marker, the per-document `Audit Report` link, `Remediation Score`, and sortable numeric `Score (0-100)` + `Grade`, plus the `Notes` staff-fill column. Open in Excel, Google Sheets, or Numbers. This is what you hand to the remediation vendor (`Notes` is for an internal pre-pass — vendors can ignore it).
